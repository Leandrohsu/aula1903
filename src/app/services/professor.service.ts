...
export class ProfessorService {
    ...
    buscarporNomeouEspecialidade(nome: string, especialidade: string): Observable<Professor[]>{
      let pars = new HttpParams().set('nome',nome).set('especialidade', especialidade);
      return this.http.get<Professor[]>(this.API+'/buscarporNomeouEspecialidade',{params:pars});
  
    }
    
    buscarPorEmail(email: string): Observable<Professor[]>{
      let pars = new HttpParams().set('email', email);
      return this.http.get<Professor[]>(this.API+'/buscarPorEmail',{params:pars});
